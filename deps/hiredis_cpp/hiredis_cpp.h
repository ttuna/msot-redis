...
public:
	...
	bool connect(const std::string &in_host, const int in_port, const bool in_blocking = true, const bool in_enable_pub_sub = false, const int in_timeout_sec = -1);
	// establish a async connection ...
	void* connectAsync(const std::string &in_host, const int in_port, RedisCallback* in_connect_callback = 0, RedisCallback* in_disconnect_callback = 0, const bool in_enable_pub_sub = false);
	// release a sync/async connection ...
	void disconnect();

	// check if server is running ...
	static bool isServerRunning();	// TODO ...
	bool isConnected() const;
	bool isPubSubEnabled() const;
	...
	RedisReply* exec(const RedisCommand& in_command) const;
	// execute a list of reis commands in pipeline mode - only available/necessary in sync blocking mode ...
	RedisReply* exec(const std::vector<RedisCommand*> &in_command_vector) const;
	// subscribe to a channel - for each message received a message callback will be called ...
	// receiving is always done in async mode ...
	void subscribe(const std::string &in_channel, RedisCallback* in_message_callback = 0, const bool in_pattern_sub = false) const;
	// subscribe to a list of channels - for each message received a message callback will be called ...
	// receiving is always done in async mode ...
	void subscribe(const std::vector<std::string> &in_channel_vector, RedisCallback* in_message_callback = 0, const bool in_pattern_sub = false) const;
	// unsubscribe a channel ...
	void unsubscribe(const std::string &in_channel, const bool in_pattern_sub = false) const;
	// unsubscribe a list of channels ...
	void unsubscribe(const std::vector<std::string> &in_channel_vector, const bool in_pattern_sub = false) const;
	// publish a message to a specific channel ...
	// sending is always done in async mode ...
	void publish(const std::string &in_channel, const std::string &in_msg) const;
	// publish a message to a list of channels ...
	// sending is always done in async mode ...
	void publish(const std::vector<::std::string> &in_channel_vector, const std::string &in_msg) const;

	// get all pending replies - reply is REDIS_REPLY_TYPE_ARRAY ...
	RedisReply* getReply() const;
	// discard all pending replies ...
	void discardReply() const;
	// free received reply ...
...
private:
	...
	bool checkCommand(const RedisCommand& in_command) const;
	// execute redis command within a specific context ...
	RedisReply* execCommand(const RedisContext& in_context, const RedisCommand& in_command) const;
	// write pending commands - only necessary in sync non-blocking mode ...
	int writePendingCommands() const;
	// get all pending replies in a vector ...
	std::vector<RedisReply*> getPendingReplies(const bool in_discard = false) const;
	// low level read from redis in-buffer ...
	int readRedisBuffer(redisReply** out_reply) const;
